class Platform {
  ...
  // 314.9, 345.1, 74.9, 105.1, 194.9, 225.1 };    // 2, 1, 6, 5, 4, 3
  //private final float baseAngles[] = {
  // 265, 335, 25, 95, 145, 215 };                   // 2, 1, 6, 5, 4, 3
  
  // UPDATED BASED ON REAL PLATFORM
  private final float baseAngles[] = {
   268.5, 330.5, 29.5, 94.5, 148.5, 210.5 };                   // 2, 1, 6, 5, 4, 3   

  //private final float platformAngles[]  = {
  // 322.9, 337.1, 82.9, 97.1, 202.9, 217.1};
  //private final float platformAngles[]  = {
  // 290, 310, 50, 70, 170, 190};                    // 170, 190, 50, 70, 310, 290
  
  // Update based on real plat 
  private final float platformAngles[]  = {
   289.17, 309.83, 51.17, 72.83, 169.17, 189.83};                    // 170, 190, 50, 70, 310, 290  
 
   //rotation of servo arms in relation to x-axis
   //  (Relates closely to the angles used for both baseAngles and platformAngles) 
   //1,      2,    3,     4, 
   //private final float beta[] = {
   //5*PI/6, PI/2, -PI/2, -5*PI/6, PI/6, -PI/6};    // ALL CORRECT!!!    
   
   private final float beta[] = {
   5*PI/6, PI/2, 3*PI/2, 7*PI/6, PI/6, 11*PI/6};    // ALL CORRECT!!!     

  // REAL MEASUREMENTS
  private final float SCALE_INITIAL_HEIGHT = 160; //  250
  private final float SCALE_BASE_RADIUS = 77.5; //140        // Measured (F360) diameter of centre of base to the servo centres (not the leg positions)
  private final float SCALE_PLATFORM_RADIUS = 63.5;          // Measured (F360) diameter of centre to bracket holes /2
  private final float SCALE_HORN_LENGTH = 30;
  private final float SCALE_LEG_LENGTH = 166; // 270
  ...
  public void draw() {
    // draw Base
//    noStroke();
//    color c0 = color(100, 160, 160);  // dark-cyan    
//    fill(c0);
    color c0 = color(123,177,219);  // NXP-Blue 
    strokeWeight(5);
    stroke(c0);
    noFill();
    ...
    for (int i=0; i<6; i++) {
      ...
      //Thin 'l'
      color c1 = color(201,210,0);  // NXP-Green       
      stroke(c1);
      strokeWeight(1);
    ...
    rotateY(rotation.y);
    rotateX(rotation.x);
    color c = color(249,181,0);  // NXP-Orange
    strokeWeight(3);
    stroke(c);
    noFill();
